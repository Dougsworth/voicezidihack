...
const CARIBBEAN_ACCENTS = {
  jamaican: {
    patterns: [
      /\bmi\b/gi, /\bdem\b/gi, /\byuh\b/gi, /\bweh\b/gi, 
      /\bdeh\b/gi, /nuh\s+true/gi, /\bseh\b/gi
    ],
    indicators: ['jamaica', 'kingston', 'spanish town', 'montego bay', 'jmd', 'patois']
  },
  trinidadian: {
    patterns: [
      /\brel\b/gi, /\btrue\s+true/gi, /\boui\b/gi, 
      /\bgyul\b/gi, /\bfete\b/gi
    ],
    indicators: ['trinidad', 'tobago', 'port of spain', 'carnival', 'ttd']
  },
  barbadian: {
    patterns: [
      /\bwunna\b/gi, /\bbrek\b/gi, /\bpun\b/gi
    ],
    indicators: ['barbados', 'bridgetown', 'bds', 'bajan']
  },
  guyanese: {
    patterns: [
      /\babie\b/gi, /\bgyaff\b/gi, /\bcotch\b/gi
    ],
...
export class CaribbeanASRService {
  ...
  private static detectAccent(text: string): CaribbeanASRResult['accent'] {
  ...
  private static analyzeSpeechPatterns(text: string): CaribbeanASRResult['speechPatterns'] {
  ...
  private static extractJobInfoCaribbean(text: string): CaribbeanASRResult['jobExtraction'] {
    ...
    // Extract budget
    const budget = this.extractBudgetCaribbean(text)
  ...
  private static extractBudgetCaribbean(text: string): CaribbeanASRResult['jobExtraction']['budget'] {
    ...
    for (const pattern of budgetPatterns) {
      ...
      if (match) {
        ...
        const currency = this.determineCurrency(text)
  ...
  private static findCaribbeanContext(text: string): CaribbeanASRResult['caribbeanContext'] {
    const lowerText = text.toLowerCase()
  ...
  private static calculateCaribbeanConfidence(
    text: string, 
